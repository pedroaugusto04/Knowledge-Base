...
test('SubscriptionService throws when a pending payment already exists', async () => {
                      ...
                      return {
                        async then(callback) {
                          if (currentIndex === 0) {
                            ...
                            return callback([mockPendingPayment]);
                          }
                          return callback([]);
                        }
                      };
  ...
  await assert.rejects(
    ...
    (err) => {
      ...
      assert.match(err.message, /subscription payment is already pending/i);
      return true;
    }
  );
});
