...
4. O callback em `KB_PUBLIC_BASE_URL + KB_GITHUB_APP_CALLBACK_PATH` valida `state`, confirma `installation_id` e prova a instalação com as credenciais do GitHub App (`KB_GITHUB_APP_ID` + `KB_GITHUB_APP_PRIVATE_KEY`) antes de vincular o workspace.
